 ...
 * @author home
 */
public class Dendo_similaridad extends JFrame {
    double esca=0;
    ...
    public Dendo_similaridad (LinkedList orden,LinkedList fus,LinkedList cut,int tipo,int punt,int xm,int sep)  {
            this.punto=punt;
            this.separacion=sep;
            ...
            int escala=1;
            this.esca=escala;
            dendoclass a=(dendoclass) fus.get(fus.size()-1);
            setTitle("Dendrograma Similaridad");
            int  xnvo=xm;
            int ynvo=(((ordenoriginal.size()*separacion))/2)+separacion*2;
            this.lim=Math.abs(((xnvo*2)-70)/(1-minimo()));
            ...
            final JPanel pnl = new JPanel() {
                ...
                public void cut(Graphics g,double s,String tx){
           ...
           public void drawEscala(Graphics g){
                    double max=simimax();
                    double inter=0;
                    double min=minimo();
                   escala(g,min,""+String.format("%.2f",min));
             escala(g,max,""+max);
                inter=max/10;
             for(int x=1;x<10;x++)
                 escala(g,inter*x,""+String.format("%.2f",inter*x));
                }
              public double simimax(){
                    double max=0;
...
//                        System.out.println(max);
                ...
                public void escala(Graphics g,double s,String tx){
                    Color c=Color.black;
                    g.setColor (c);
...
//                        if(a.getValue()==0){
//                            g.fillOval((int) (x),ynvo-a.getY()-4, 5, 8);
//                        }
//                        else{
                        g.fillOval((int) (x+ajustedendo(a.getValue())),ynvo-a.getY()-4, 5, 8);
                        //}
                    }
                }
...
// separar tokens y colocar en una lista, recrear cadena hasta que cadena no se encuentre dipononoble
                ...
                public  LinkedList find(String name){
                    ...
                    // Value.show(this.orden);
                    return null;
                }
                public void  Dawcu(Graphics g,String a,String b,int seme,int t){
                    int y=0;
                ...
                public void addrec(Graphics g,double x, double y,double ancho,int largo){
                    g.fillRect(xnvo+1*(int)x,ynvo-1*(int)y,largo, (int) ancho);
                    
                }
                public void addrec1(Graphics g,double x, double y,double ancho,int largo){
                    g.drawRect(xnvo+1*(int)x,ynvo-1*(int)y,largo, (int) ancho);
                    
                }
                public void addtext(Graphics g,String text,double x, double y,int scala){
                    g.drawString(text, xnvo+scala*(int)x,ynvo-scala*(int)y);
                    
                }
                
                public void eje(Graphics g){
                    g.setColor (Color.black);
                ...
                public void eje(){
                    for(int x=0;x<ordenoriginal.size();x++){
                        ...
                        a.add(0);
                        this.puntos.add(new point(0,(ynvo-separacion*(x+1))-20,1));
